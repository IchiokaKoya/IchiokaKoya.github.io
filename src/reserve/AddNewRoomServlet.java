...
public class AddNewRoomServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			meetingRoom.newRoomRegistration(room);
			request.getRequestDispatcher("registeredRoom.jsp").forward(request, response);
		} catch (Exception e) {
			errorReason = e.getMessage();
			request.setAttribute("errorReason", errorReason);
			request.getRequestDispatcher("registeredRoomError.jsp").forward(request, response);
		}
	}
}
